...
namespace vio {
	 ...
	 * VecX residual_;                 // 残差
	 * std::vector<MatXX> jacobians_;  // 雅可比，每个雅可比维度是 residual * vertex[i]
	...
	*/
		...
		///是世界坐标系下的三维点作为参数，而不是逆深度，只有相机参数参与优化
		...
		void EdgeReprojectionXYZ::SetTranslationImuFromCamera(const Eigen::Quaterniond &qic_,const Vec3 &tic_) {
			qic = qic_;
			tic = tic_;
		}
