#include "backend/imu_intergration.h"
...
namespace vio {
	namespace backend {
		void IMUIntegration::Propagate(double dt, const Vec3 &acc, const Vec3 &gyr) {
			...
			///时间每过dt，就用类似于欧拉法更新i时刻导j时刻的delta_p、delta_v、delta_q
			...
			// update jacobians q v p with bg and ba
			...
			// propagate noise A和B即噪声协方差传播的F和V
